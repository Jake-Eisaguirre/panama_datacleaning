...
librarian::shelf(tidyverse, here, janitor, lubridate, parsedate, data.table, anytime)
...
t <- pan_data %>% 
  select(date, region, site, utmz, utme, utmn) %>% 
  filter(region == "Fortuna",
         site == "Alleman")
...
time_check <- pan_data %>% 
  select(region, site, date, start_time, end_time, capture_time, swab_id)

greater_time_error <- time_check %>% 
  filter(capture_time > end_time) %>% 
  mutate(correct_capture_time = "")

write.csv(greater_time_error, here("greater_time_error.csv"))

less_time_error <- time_check %>% 
  filter(capture_time < start_time) %>% 
  mutate(correct_capture_time = "")

write.csv(less_time_error, here("less_time_error.csv"))
```
...
sal <- var_environ %>%
  filter(salinity_large > 0.1) %>%
  select(salinity_large, swab_id, survey_id, region, site, date, detection_type, survey_time) %>%
...
id_check <- pan_data %>% 
  select(region, site, date, detection_type, survey_time, species, swab_id) %>% 
...
conductiity <- var_environ %>% 
  select(conductivity_insertunits) %>% 
  unique()
